...
def test_union():
    ...
    d2 = session.dataset("test2")
    d3 = session.dataset("test3")
    d4 = d1.where(d1.code > 33).where(d1.name.isin(["k"]))
    res = session.union([d1, d2, d3, d4])
    print(res.to_sql())


@pytest.mark.parametrize("database", database_urls, ids=database_names)
def test_colums_meta(database):
    connection = create_engine(database)
    session = Session(connection)
    ...
    df = pd.DataFrame(np.array([[1, 2.3, 'A'], [4, 5.7, 'B'], [7, 8.0, 'B']]),
                      columns=['a', 'b', 'd'])
    to_sql(df, "test_table_2", con=connection, index=False)
    df = pd.DataFrame(np.array([[1, 2.3, 'A'], [4, 5.7, 'B'], [7, 8.0, 'B']]),
                      columns=['a', 'e', 'f'])
    to_sql(df, "test_table_3", con=connection, index=False)
    test_table = session.dataset("test_table")
    test_table_2 = session.dataset("test_table_2")
    test_table_3 = session.dataset("test_table_3")
    joined_table = test_table.join(test_table_2).using(col("a")).join(test_table_3).on(test_table.a==test_table_3.a)
...
def test_colums_meta(database):
    ...
    df = pd.DataFrame(np.array([[1, 'A', 0], [4, 'B', 1], [7, 'B', 0.4]]),
                      columns=['patient_id', 'category', 'value'])
    to_sql(df, "test_table", con=connection, index=False)
    test_ds = session.dataset("test_table")
    ohe = OneHotEncoder(input_cols='category')
    ohe.fit(test_ds)
    trans = ohe.transform(test_ds)
    print(trans)
